...
void Team2::CreateCharacter(int x, int y)
{
	...
	for (int i = 0; i < MAX_OBJECT__COUNT; ++i)
	{
		if (pObject[i] == NULL)
		{
			...
			{
			case 0:
				pObject[i] = new Character(OBJECTTYPE::CHARACHTER, tag, MyVector(x, y, 0), FindTarget(), MyColor(1, 1, 1, 1), 30.0f, 100.0f, 9999999.0f, 100.0f);
				return;
			case 1:
				pObject[i] = new CharacterArrow(OBJECTTYPE::CHARACHTER_ARROW, tag, MyVector(x, y, 0), FindTarget(), MyColor(1, 1, 1, 1), 30.0f, 300.0f, 9999999.0f, 10.0f);
				return;
			case 2:
				return;
			}
